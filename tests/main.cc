...
bool test_AES()
{
    ...
    // init AES context.
    cout << "AES init: " << AES_init(key, keylen, 0, 128, ctx) << "\n";
...
bool test_RSA()
{
    ...
    cout << "generate_key: " << RSA_gen_keys("public.pem", "private.pem", 4096, 1, (BYTES)"private key encryption passphrase", 33) << "\n";
    ...
    cout << "pubkey init: " << RSA_key_file_init("public.pem", 0, PUBLIC_KEY, ctx) << "\n";
    cout << "privkey init: " << RSA_key_file_init("private.pem", (BYTES)"private key encryption passphrase", PRIVATE_KEY, ctx) << "\n";
